...
9. No empty `catch` blocks in try/catch. A `catch` block must minimally include a comment as to why the catch is empty, e.g.
   ...
   ```
10. All `switch` statements must include a `defaut` clause.  Also, each clause in a `switch` must include a `break`, `return`, `throw` or `continue` (no [fall through](https://checkstyle.sourceforge.io/checks/coding/fallthrough.html#FallThrough) allowed), e.g.
   ```
